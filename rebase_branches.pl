...
say "\nNot koha-plugin-edifact-brodart, exiting" && exit 0 unless $repo eq "koha-plugin-edifact-brodart";
...
foreach my $repo (@repos) {
    next if $repo eq 'koha-plugin-edifact-brodart';
    next if $repo eq 'koha-plugin-edifact-brodart-docs';
